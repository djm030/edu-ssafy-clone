...
- Passing evidence: Dockerized backend Maven Java 21 tests (`53` tests), `mvn -B prepare-package`, frontend typecheck/lint/build, compose config render, app-profile rebuild/healthy containers, backend/nginx container-internal docs/API smoke, static API Docs/OpenAPI JSON/YAML validation, controller-vs-API Docs drift check, repository-vs-schema table check.
- Failing/partial evidence: host published-port curl cannot connect from this agent sandbox, Spring REST Docs covers only 4 of 52 controller operations, and browser E2E/visual evidence is absent. Springdoc/Swagger UI and `/v3/api-docs` are optional future runtime docs, not current blockers.
- Product blockers still tracked: production auth/session/RBAC, binary attachments, support-ticket answer/thread/status/attachments, survey/notification lifecycle depth, Spring REST Docs coverage, team recovery/commit hygiene.
